...
pair<int, bool> func(int oxy, int nit, int i, int n, pair<int, pair<int, int>> gas[]) {
    if (i >= n) return {INT32_MAX/2, false};
    if (oxy < 0) oxy = 0;
    if (nit < 0) nit = 0;
    ...
    if(oxy-gas[i].first <= 0 and nit - gas[i].second.first <=0){
        ...
        if (i + 1 == n) ans1.first = gas[i].second.second;
        else ans1.first += gas[i].second.second;
    }
...
void solve() {
    test {
        ...
        cout << (func(oxy, nit, 0, n, gas).second? func(oxy,nit, 0, n, gas).first: 1000000 )<< endl;
    };
}
